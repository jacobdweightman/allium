...
//
// PROTECT is required as an additional workaround for MSVC++, because extra
// parentheses are aparently not allowed around function arguments. 
//
...
#define DECODE_UNION(OBJECT, PROPERTY_NAME, ...) \
    DECODE_PROPERTY_WITH_DECODER(OBJECT, PROPERTY_NAME, PROTECT(TaggedUnion<__VA_ARGS__>), (decodeJSONUnion<__VA_ARGS__>))
...
Optional<JSONRPCRequest> JSONRPCRequest::decode(JSON json) {
    return json.as_a<JSONObject>().flatMap<JSONRPCRequest>([](JSONObject o) {
        ...
        DECODE_UNION(o, id, std::string, int, Unit);
...
Optional<InitializeParams> InitializeParams::decode(JSON json) {
    return json.as_a<JSONObject>().flatMap<InitializeParams>([](JSONObject o) {
        DECODE_UNION(o, processId, int, Unit);
